...
(function() {
    ...
    function flattenEvents(phases) {
        var events = [];
        phases.forEach(function(phase) {
            (phase.events || []).forEach(function(event) {
                events.push(Object.assign({}, event, {
                    phaseTitle: phase.title,
                    anchorYear: event.anchorYear || inferAnchorYear(event.date)
                }));
            });
        });
        events.sort(function(a, b) {
            return a.anchorYear - b.anchorYear;
        });
        return events;
    }

    function initGoldenAgeEvents() {
        var root = document.getElementById('golden-age-events-root');
        var phases = window.BAGHDAD_GOLDEN_AGE_PHASES || [];
        if (!root || !phases.length) return;

        var events = flattenEvents(phases);
        var lastYear = null;
        root.innerHTML = phases.map(function(phase) {
            ...
            var cards = sortedEvents.map(function(event) {
                ...
                var media = event.mediaPlaceholder
                    ? '<div class="golden-event-media">' + event.mediaPlaceholder + '</div>'
                    : '';
                return [
                    '<article class="golden-event' + (event.highlight ? ' is-highlight' : '') + '" data-anchor-year="' + anchorYear + '" style="margin-top:' + marginTop.toFixed(1) + 'px">',
                    '<div class="golden-event-date">' + event.date + '</div>',
                    '<div>',
                    '<div class="golden-event-phase-label">' + phase.title + '</div>',
                    '<h4 class="golden-event-title">' + event.title + '</h4>',
                    '<p class="golden-event-copy">' + event.description + '</p>',
                    media,
                    '</div>',
                    '</article>'
                ].join('');
    ...
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initGoldenAgeEvents);
    else initGoldenAgeEvents();
})();
...
(function() {
    var eras = [
        { label: 'Abbasid Caliphate',   years: '762-945',  color: '#1a6b4a', start: 762,  end: 945  },
        { label: 'Buyid Control',       years: '945-1055', color: '#6f7c35', start: 945,  end: 1055 },
        { label: 'Seljuk Sultanate',    years: '1055-1157',color: '#80613b', start: 1055, end: 1157 },
        { label: 'Late Abbasid Revival',years: '1157-1258',color: '#b3842f', start: 1157, end: 1258 },
