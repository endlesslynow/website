...
TAG_RE = re.compile(r'<span class="tag">(.*?)</span>', re.DOTALL)
...
def tag_to_link(match: re.Match[str], file_link_map: dict[str, str]) -> str:
    label_html = match.group(1)
    label = collapse_whitespace(html.unescape(re.sub(r"<[^>]+>", "", label_html)))
    href = file_link_map.get(normalize_lookup_key(label), "kurdish_research.html")
    return f'<a class="tag" href="{html.escape(href, quote=True)}">{label_html}</a>'
...
def link_used_file_tags(text: str, file_link_map: dict[str, str]) -> tuple[str, int]:
    ...
    def replace(match: re.Match[str]) -> str:
        nonlocal linked
        replacement = tag_to_link(match, file_link_map)
